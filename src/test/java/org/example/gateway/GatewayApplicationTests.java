...
class GatewayApplicationTests {
	...
	void contextLoadsConfigServerTest() {
		ResponseEntity<String> response = restTemplate.getForEntity("http://localhost:8888/gateway/default", String.class);
		Assertions.assertEquals(HttpStatus.OK, response.getStatusCode());
		Assertions.assertTrue(response.getBody().toLowerCase().contains("gateway"));
	}

	@Test
	void contextRegisterEurekaServerTest() throws InterruptedException {
		Thread.sleep(14000);
